...
walkLength=[4,5,6,7]
repeat=10
resultArray=np.zeros([len(nodesNumber)*len(P)*len(pAlpha)*len(pEdges)*len(walkLength),17])
for a in range(len(nodesNumber)):
    for b in range(len(P)):
        graphMain = nx.fast_gnp_random_graph(n=nodesNumber[a], p=P[b])
        for c in range(len(pEdges)):
            auc_adamic = 0
            auc_jaccard = 0
            auc_resource_allocation = 0

            for iter in range(repeat):
                proportion_edges = pEdges[c]
                ...
                score_resource_allocation, label_resource_allocation = zip(
                    *[(s, (u, v) in edge_subset) for (u, v, s) in pred_resource_allocation])
                try:
                    fpr_resource_allocation, tpr_resource_allocation, _ = metrics.roc_curve(label_resource_allocation,
                                                                                            score_resource_allocation)
                    t_auc_resource_allocation = metrics.roc_auc_score(label_resource_allocation,
                                                                      score_resource_allocation)
                except:
                        ...
                        for g in range(len(walkLength)):
                            ...
                            for iter in range(repeat):
                                for h in range(len(methods)):
                                        methodH = methods[h]
                                        node2vec = Node2Vec(graph,edge_subset,  dimensions=8, walk_length=walkLength[g], num_walks=numWalks[f],p=pAlpha[d],q=1/pAlpha[d], workers=4,method=methodH,directed=False)
